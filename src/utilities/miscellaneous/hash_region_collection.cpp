#include "hash_region_collection.h"
...
void HashRegionCollection::Init(const BestRegionArray& array) {
...
void HashRegionCollection::SortByLength() {
  sort(hash_regions_.begin(), hash_regions_.end(), OperatorLength);
}

const BestRegion* HashRegionCollection::Get (const unsigned int& index) const {
  if (index > hash_regions_.size()) return NULL;

  return hash_regions_[index];
}

void HashRegionCollection::Print(void) const {
